...
public class ShowKeyCommandThread extends Thread {
    ...
    public static void listRideVehicle() {
        if (Minecraft.getInstance().player != null) {
            ...
            String a = new TranslatableComponent("show_key.Now.riding").getString().replace("%s", "");
    ...
    private static MutableComponent getCopyComponent(String string, String copyString) {
        MutableComponent component = Component.nullToEmpty(copyString).copy().withStyle(Style.EMPTY.withUnderlined(true).withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, copyString))
                .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TranslatableComponent("tips.show_key.click_to_copy"))));
        if (!string.isEmpty()) {
            return new TranslatableComponent(string).append(" ").append(component);
        } else {
            return component;
        }
    }
